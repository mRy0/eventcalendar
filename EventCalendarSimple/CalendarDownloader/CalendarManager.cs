...
namespace EventCalendar.CalendarDownloader
{
    public class CalendarManager :IDisposable
    {
        ...
        public static Ical.Net.DataTypes.CalDateTime ToIcalDate(DateTime time, bool round = false)
        {
            if (round)
            {
                time = time.Date.AddHours(time.Hour).AddMinutes(time.Minute);
            }
